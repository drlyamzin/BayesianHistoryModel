function [prob_out,init_time,bslprep_time,bsloop_time,half_time,full_time,int_time]=calcBayesProb(mu,r,s,params,mode,bessel_table,bessel_coords)
...
%unpack
a = params(1);% stimulus history
b = params(2);% choice history
sgmax = params(3);% sigma for xcalcBayesOffsetProbLookupTable_Benchmark4
sgmay = params(4);% sigma for y
sgmapr = params(5);% sigma of history "prior"
if length(params)==5
    x0 = 0; y0 = 0;
else
    x0 = params(6);% perceptual bias for x
    y0 = params(7);% perceptual bias for y
end
...
if strcmp(mode,'CPU'),
    ...
    h = a*s+b*r;% the way history terms appear in calculations
    ...
    tmp3 = bsxfun(@times,cos(X)-cos(Y),this_hist*sgmapr) + tmp12; clear tmp12 

    % VI. calculate a coefficient for the full expression
    C1C2C3 = 1./(2*pi*besx * 2*pi*besy * (2*pi*bespr).^2 );
